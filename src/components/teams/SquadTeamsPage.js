...
const SquadTeamsPage = () => {
   const {squadteamId} = useParams();
    const [squadteam, setSquadTeam] = useState(null);
    
    useEffect( () => {
        const fetchSquadTeam = async () => {
            try {
                const response = await SquadTeamsService.fetchSquadTeamById(squadteamId);
                setSquadTeam(response.data);
            } catch (e) {
                console.log(e);
            }
        };
                            ...
                            <div className="statistics-row">
                                <div className="statistics-col">
                                    <div className="statistics-box">
                                        ...
                                        <h6> {squadteam.statistics.amountGame} </h6>
                                    </div>
                                    <div className="statistics-box">
                                        <img src={KillIcon} alt="kills"/>
                                        <h6> {squadteam.statistics.amountKill} </h6>
                                    </div>
                                    <div className="statistics-box">
                                        <img src={MoneyIcon} alt="money"/>
                                        <h6>  {squadteam.statistics.sumPrizeMoney} </h6>
                                    </div>
                                </div>
                                <div className="statistics-col">
                                    <div className="statistics-box">
                                        <img src={FirstPlaceIcon} alt="top-1"/>
                                        <h6>  {squadteam.statistics.amountFirstPlace} </h6>
                                    </div>
                                    <div className="statistics-box">
                                        <img src={SecondPlaceIcon} alt="top-2"/>
                                        <h6>  {squadteam.statistics.amountSecondPlace} </h6>
                                    </div>
                                    <div className="statistics-box">
                                        <img src={ThirdPlaceIcon} alt="top-3"/>
                                        <h6>  {squadteam.statistics.amountThirdPlace} </h6>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default observer(SquadTeamsPage);